...
lat_accel_data = S.lat_accel(:, 2);
lat_accel_time = S.lat_accel(:, 1);
long_accel_data = S.long_accel(:, 2);
long_accel_time = S.long_accel(:, 1);
vert_accel_data = S.vert_accel(:, 2);
vert_accel_time = S.vert_accel(:, 1);

hold on
plot(lat_accel_time, lat_accel_data, '.-');
plot(long_accel_time, long_accel_data, '.-');
plot(vert_accel_time, vert_accel_data, '.-');
grid on

xlabel('Time (s)')
ylabel('Lateral, Longitudal, Vertical Acceleration (m/s^2)')
title('IMU Accelerometer')
legend({'Lateral Acceleration','Longitudal Acceleration', 'Vertical Acceleration'})
