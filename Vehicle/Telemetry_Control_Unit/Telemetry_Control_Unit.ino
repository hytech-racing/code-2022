 ...
 *
 * @brief: Logs CAN Bus data to an SD card. Filters messages to be sent to XBee via UART for wireless transmission.
 * 
 * @authors:
 * Ryan Gallaway
 * Nathan Cheek
...
#include <kinetis_flexcan.h> // Used to access FlexCAN config params to allow self-reception; otherwise HyTech_FlexCAN does the work
#include <Wire.h>
#include <TimeLib.h>
#include <Metro.h>
#include <XBTools.h>
...
// Required to be registered as callback by setSyncProvider
inline time_t getTeensy3Time() {
    return Teensy3Clock.get();
}

/**
 * @brief: Set up resources and initialize SD card communication
 */
void setup() {
...
/**
 * @brief: Helper function to parse CAN message from Vehicle CAN line
 */ 
void parse_can_message() {
...
/**
 * @brief: Writes message buffer to SD card. Performs pseudo-millisecond algorithm calculation.
 * Note: This function does not flush data to disk! It will happen when the buffer fills or when the above flush timer fires
 */ 
void write_to_SD(CAN_message_t *msg) {
    // Calculate Time - Pseudo-millisecond Algorithm
...
/**
 * @brief: Software filter on ADC inputs
 */ 
...
/**
 * @brief: Process MCU analog reading calculations
 */ 
...
/**
 * @brief: Low-level XBee message transmission helper function
 */ 
int write_xbee_data() {
    /*
...
/**
 * @brief: Selects CAN messages due to be sent to XBee
 */ 
void send_xbee() {
...
/**
 * @brief: SD card callback function (required by API)
 */
void sd_date_time(uint16_t* date, uint16_t* time) {
