/*
 * 2019 Cell Charging Code (modified HyTech 2019 BMS Control Unit)
 * Init 2017-04-11
 * Configured for HV Board Rev 7
 ...
 */
#define TOTAL_IC 16                      // Number of ICs in the system
...
void process_cell_temps() { // TODO make work with signed int8_t CAN message (yes temperatures can be negative)
    ...
    for (int ic = 0; ic < TOTAL_IC; ic++) {
        for (int j = 0; j < THERMISTORS_PER_IC; j++) {
            ...
            ignore_cell_therm[ic][j] = 1; // ignore all thermmistors because they are not used on the charging board
            ...
            if (!ignore_cell_therm[ic][j]) {
                ...
                Serial.println(" C");
 /*           } else {                  // commmented out, so that the Serial output is not cluttered
                Serial.print("IC ");
                Serial.print(ic);
                Serial.print(" CELL Thermistor ");
                Serial.print(j);
    ...
    for (int ic = 0; ic < TOTAL_IC; ic++) {
        for (int j = 0; j < PCB_THERM_PER_IC; j++) {
            ...
            if (!ignore_pcb_therm[ic][j]) {
                ...
                Serial.print(thermTemp / 100, 2);
                Serial.println(" C");
/*            } else {                    // commented out so that the Serial output is not cluttered
...
void print_cells() {
    for (int current_ic = 10; current_ic < 12; current_ic++) { // only print ICs 10 and 11
        Serial.println();
        Serial.print("IC: ");
