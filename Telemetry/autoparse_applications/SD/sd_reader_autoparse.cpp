...
void run (FILE* infile, FILE* __outfile) {
	...
	uint64_t timeRaw;
	#ifdef FORMAT_DATESTRING
		#define DATE_INPUTTYPE "%s"
		uint64_t* timeInputPtr = &timeRaw;
	#else
		#define DATE_INPUTTYPE "%lu"
		char* timeInputPtr = timeString;
	#endif

	while (fscanf(infile, DATE_INPUTTYPE ",%x,%lx", timeInputPtr, &id, (uint64_t*) data) != EOF) {
		#ifdef FORMAT_DATESTRING
			int ms = timeRaw % 1000;
			timeRaw /= 1000;
			size_t len = strftime(timeString, 32, "%Y-%m-%d %H:%M:%S", gmtime((time_t*) &timeRaw));
			sprintf(timeString + len, ".%d", ms);
		#endif
